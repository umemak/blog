---
title: "RSS棚卸し"
date: "2020-02-29"
tags: ["rss"]
---

RSSリーダーをNewsifyからInoreaderに移行するにあたって、無料プランだと150件までしか登録できないので、更新されていないものなど削除して棚下ろす。

Inoreaderに移行する理由は、NewsifyのAndroidアプリが存在しないため。

## 日本のニュース
* [ITmedia 総合記事一覧](https://rss.itmedia.co.jp/rss/2.0/itmedia_all.xml)
* [ビジネス+IT 最新記事](https://www.sbbit.jp/index1.rss)
* [ZDNet Japan 最新情報　総合](http://feed.japan.zdnet.com/rss/news/index.rdf)
* [TechCrunch Japan](https://jp.techcrunch.com/feed/)
* [Publickey](https://www.publickey1.jp/atom.xml)
* [CNET Japan 最新情報　総合](http://feed.japan.cnet.com/rss/index.rdf)
* [Think IT（シンクイット）](https://thinkit.co.jp/rss.xml)
* [CodeZine:新着一覧](https://codezine.jp/rss/new/20/index.xml)
* [＠IT 全フォーラム 最新記事一覧](https://rss.itmedia.co.jp/rss/1.0/ait.xml)
* [InfoQ](https://feed.infoq.com/jp)

## Software Engineering
* [Google Developers Blog](http://feeds.feedburner.com/blogspot/Dcni)
* [Hacker News](https://news.ycombinator.com/rss)
* [Android Developers Blog](http://feeds.feedburner.com/blogspot/hsDu)
* [reddit:programming](https://www.reddit.com/r/programming/.rss)
* [reddit:Technology](https://www.reddit.com/r/technology/.rss)
* [Ruby Weekly](http://rubyweekly.com/rss)

## Tech News
* [Lifehacker](http://lifehacker.com/rss)
* [The Verge](http://www.theverge.com/rss/index.xml)
* [Engadget](http://www.engadget.com/rss.xml)

## gcp
* [Google Cloud Platform](https://cloudblog.withgoogle.com/ja/products/gcp/rss/)
* [Google Developer Japan Blog](http://googledevjp.blogspot.com/feeds/posts/default)
* [Google Cloud Platform](https://cloudplatform.googleblog.com/feeds/posts/default?alt=rss)
* [Google Cloud Platform Japan 公式ブログ](http://feeds.feedburner.com/GoogleCloudPlatformJapanBlog)
* [Cloud Blog](https://cloudblog.withgoogle.com/rss/)

## azure
* [Microsoft Azure Blog](https://azure.microsoft.com/en-us/blog/feed/)
* [Azure service updates](https://azure.microsoft.com/en-us/updates/feed/)
* [Microsoft Azure Blog &gt; Announcements](https://azure.microsoft.com/en-us/blog/topics/announcements/feed/)
* [Azure InfoHub RSS - Azure](https://azureinfohub.azurewebsites.net/Feed?serviceTitle=Azure)

## aws
* [aws blog](https://aws.amazon.com/about-aws/whats-new/recent/feed/)
* [AWS Database Blog](https://aws.amazon.com/blogs/database/feed/)
* [Amazon Web Services](https://www.youtube.com/feeds/videos.xml?user=AmazonWebServices)
* [AWS Podcast](https://d3gih7jbfe3jlq.cloudfront.net/aws-podcast.rss)
* [Amazon Web Services Japan 公式](https://www.youtube.com/feeds/videos.xml?user=AmazonWebServicesJP)
* [AWS Online Tech Talks](https://www.youtube.com/feeds/videos.xml?user=AWSwebinars)
* [Amazon Linux Security Center](https://alas.aws.amazon.com/alas.rss)
* [AWS Cloud Enterprise Strategy Blog](https://aws.amazon.com/blogs/enterprise-strategy/feed/?_=5597)
* [AWS News Blog](https://aws.amazon.com/blogs/aws/feed/)
* [AWS Architecture Blog](https://aws.amazon.com/blogs/architecture/feed/)
* [AWS Partner Network (APN) Blog](https://aws.amazon.com/blogs/apn/feed/)
* [AWS Marketplace](https://aws.amazon.com/blogs/awsmarketplace/feed/)
* [AWS Big Data Blog](https://aws.amazon.com/blogs/big-data/feed/)
* [AWS Compute Blog](https://aws.amazon.com/blogs/compute/feed/)
* [AWS Developer Blog](https://aws.amazon.com/blogs/developer/feed/)
* [AWS DevOps Blog](https://aws.amazon.com/blogs/devops/feed/)
* [Amazon Game Tech Blog](https://aws.amazon.com/blogs/gametech/feed/)
* [The Internet of Things on AWS – Official Blog](https://aws.amazon.com/blogs/iot/feed/)
* [AWS Machine Learning Blog](https://aws.amazon.com/blogs/machine-learning/feed/)
* [AWS Management &amp; Governance Blog](https://aws.amazon.com/blogs/mt/feed/)
* [AWS Media Blog](https://aws.amazon.com/blogs/media/feed/)
* [AWS Messaging &amp; Targeting Blog](https://aws.amazon.com/blogs/messaging-and-targeting/feed/)
* [AWS Mobile Blog](https://aws.amazon.com/blogs/mobile/feed/)
* [Networking &amp; Content Delivery](https://aws.amazon.com/blogs/networking-and-content-delivery/feed/)
* [AWS Open Source Blog](https://aws.amazon.com/blogs/opensource/feed/9
* [AWS Government, Education, &amp; Nonprofits Blog](https://aws.amazon.com/blogs/publicsector/feed/)
* [AWS for SAP](https://aws.amazon.com/blogs/awsforsap/feed/)
* [AWS Security Blog](https://aws.amazon.com/blogs/security/feed/)
* [AWS Startups Blog](https://aws.amazon.com/blogs/startups/feed/)
* [Amazon Web Services ブログ](https://aws.amazon.com/jp/blogs/news/feed/)

## 企業blog
* [Developers.IO](https://dev.classmethod.jp/feed/)
* [アシアルブログ](https://blog.asial.co.jp/rss)
* [さくらインターネット研究所](https://research.sakura.ad.jp/feed/)
* [Mercari Engineering Blog](http://tech.mercari.com/feed)
* [Engineers' Blog](https://moneyforward.com/engineers_blog/feed/)
* [Future Tech Blog - フューチャーアーキテクト](https://future-architect.github.io/atom.xml)
* [sansan builders box](https://buildersbox.corp-sansan.com/feed)
* [TechRacho](https://techracho.bpsinc.jp/feed/atom)
* [エムスリーテックブログ](https://www.m3tech.blog/feed)
* [クックパッド開発者ブログ](https://techlife.cookpad.com/feed)
* [Yahoo! JAPAN Tech Blog](https://techblog.yahoo.co.jp/index.xml)
